...
class CatalogController(Controller):
	...
	@get("/")
	async def get_cards(self, redis: Redis = Depends(get_redis_client)):
	...
	async def new_basket_state(self, basket: Basket, redis: Redis = Depends(get_redis_client), user: User = Depends(authorize_user)):
		for product in basket.products:
			if not product.count > 0:
				basket.products.remove(product)
		await redis.set(f"{RedisDB.basket}:{user.id}", basket.model_dump_json())
		pr = ProductRepository(self.session)
		products = await pr.get_all()
		products_dict = dict()
		...
		keys = list(await redis.scan_iter(match=prefix))
		...
		await redis.set(f"{RedisDB.free_workers}", Config.worker_count - locked_workers) 
		return {"message": "OK"}

	@get("/basket")
	async def get_basket(self, redis: Redis = Depends(get_redis_client), user: User = Depends(authorize_user)):
