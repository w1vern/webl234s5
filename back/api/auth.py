...
class AuthController(Controller):
    ...
    async def login(self,
        ...
        await redis.set(f"{RedisDB.auth_session}:{session_id}",
                  str(user.id), ex=Config.auth_session_lifetime)
        return {"message": "OK"}
    ...
    @post("/logout")
    async def logout(self, response: Response, redis: Redis = Depends(get_redis_client), session_id: str = Cookie(default=None)):
        response.set_cookie(key='session', value='', max_age=0, httponly=True)
        await redis.delete(f"{RedisDB.auth_session}:{session_id}")
        return {"message": "OK"}

    @get("/session")
    async def get_session(self, session_id: str = Cookie(default=None), redis: Redis = Depends(get_redis_client)):
